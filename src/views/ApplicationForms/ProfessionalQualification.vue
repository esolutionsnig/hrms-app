<template>
  <div class="dashboard">
    <h2 class="grey--text primary--text text-uppercase display-1">Professional Qualification</h2>
    <h4 class="subheading">View and manage your professional qualification information</h4>
...
export default {
  ...
  methods: {
    ...
    getProfessionalQualifications() {
      Axios.get(`${config.apiUrl}/users/${this.$root.curuser.id}/professionalqualifications`)
        .then(response => {
          if (response.data.data.length != 0) {
            this.$root.curuserpe = response.data.data;
    ...
    saveRecord() {
      if (this.$refs.saveForm.validate()) {
        ...
        this.disabled = 1
        Axios.post(
          `${config.apiUrl}/users/${this.$root.curuser.id}/professionalqualifications`,
          {
          ...
          .then(response => {
            ...
            this.disabled = 0
            console.log(response.data);
            this.$noty.success("Applicant Professional Qualification Successfully Created.");
            this.getProfessionalQualifications()
    ...
    deleteRecord(id) {
      if (id) {
        this.deleting = true;
        Axios.delete(
          `${config.apiUrl}/users/${this.$root.curuser.id}/professionalqualifications/${id}`,
          {
            headers: {
              Authorization: `Bearer ${this.$root.auth.access_token}`
            }
          ...
          .then(response => {
            this.deleting = false;
            this.$noty.success("Applicant Professional Qualification Successfully Deleted.");
            console.log(response.data);
            this.getProfessionalQualifications()
            setTimeout(() => location.reload(), 2000);
          })
